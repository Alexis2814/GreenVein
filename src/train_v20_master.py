...
def train_v20_decentralized(n_episodes=500, max_t=800, eps_start=1.0, eps_end=0.01, eps_decay=0.98):
    ...
    for i_episode in range(1, n_episodes + 1):
        print(f"\n🚀 --- VÒNG {i_episode}/{n_episodes} | Epsilon: {eps:.3f} ---", flush=True)
        ...
        # 🌟 BIẾN ĐỂ KIỂM TRA LÝ DO KẾT THÚC
        ...
        # 🌟 IN RA LÝ DO KẾT THÚC
        print(f'\n📊 BẢNG TỔNG KẾT EPISODE {i_episode}:', flush=True)
        if finished_early:
            print("🏁 Trạng thái: Hoàn thành sớm (Đã dọn sạch rác).")
        else:
            print("⏳ Trạng thái: Hết ca làm việc (Timeout).")
    ...
    plt.title(f'TIẾN TRÌNH HUẤN LUYỆN V20 PRO {n_episodes} VÒNG (3 Không Độc Lập)', fontsize=16, fontweight='bold')
